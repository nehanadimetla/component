...
The representation for FlashcardDeck1L is a Sequence of Card objects paired with an int currentIndex, where Card is a private inner class holding front and back strings. I chose Sequence because a flashcard deck is inherently ordered and requires indexed access, making it a natural fit for navigation and insertion/removal.
Convention: this.deck is never null; this.currentIndex is in [0, this.deck.length() - 1] when non-empty, and 0 when empty; no card or its fields are null.
Correspondence: this represents the ordered sequence of cards in this.deck, where the current card is the entry at this.currentIndex.
